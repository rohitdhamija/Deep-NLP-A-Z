...
# creating a lit of all conversations
conversations_ids = []        
...
#creating a dictionary that maps each word to it's occurances
word2count = {}
...
#creating two dictionaries that map the questions words and answer to a unique integer
threshold = 20
questionswords2int = {}
word_number = 0
...
# Translate all question and answer into integers
...
##### PART 2 - Building the SEQ2SEQ MODEL ##########3
...
# targets are the answers . target must be into matches. feed nn with batches of answer
# each answer must start with sos. unique integer at the beginning inside each batch
# create batch and add sos token
def preprocess_targets(targets, word2int, batch_size):
    left_side = tf.fill([batch_size, 1], word2int['<SOS>'] )
    right_side = tf.strided_slice(targets, [0,0], [batch_size, -1], [1,1])
    preprocessed_targets = tf.concat([left_sides, right_side], 1)
...
# stacked LSTM
# function that will define encoder of RNN layer
#    
...
# we got attentional decoder function    
def decode_training_set(encoder_state, decoder_cell, 
    ...
    attention_keys, attention_values, attention_score_function, attention_construct_function = tf.contrib.seq2seq.prepare_attention(attention_states, attention_options = 'bahdanau',
...
# Setting the sequence length to maximum length which will be 25
sequence_length = tf.placeholder_with_default(25, None, name = 'sequence_length')
    
#Getting the shape of the input tensors, we have set the hyper parameters, set some values for first try
#set the interactive session, lr, keep_prob, seq_len one of arg is sequence length
#shape of input, as it will be argument for training 
...
# split the data into batches of question and answers
